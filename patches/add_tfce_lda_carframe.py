...
# replace the existing TFCE-LDA car-frame code cell (and its md) if present
code_i = md_i = None
for i,c in enumerate(nb["cells"]):
    s = "".join(c["source"]) if isinstance(c["source"],list) else c.get("source","")
    if c["cell_type"]=="code" and "TFCE-LDA (CAR FRAME)" in s: code_i=i
    if c["cell_type"]=="markdown" and "Decodability over time (car frame)" in s: md_i=i

md = r'''## Decodability over time (car frame): Base-vs-Full and Success-vs-Failure

Per-timepoint LDA decoding with cluster-based TFCE permutation testing, car-frame
variables, `car_reference` CSVs. Six analyses:

1. **Base vs Full** -- unbalanced (all) and balanced (equal success/failure ratio per arm)
2. **Success vs Failure** -- unbalanced (all) and balanced (equal condition mix per arm)
3. **Success vs Failure, Base only**
4. **Success vs Failure, Full only**

Balanced versions stratify out the OTHER factor's confound. Raw car-frame
values (no operation C: a static offset is legitimate decodable signal).

Power warning: Base-only and Full-only outcome tests have small per-class n
(Base-success ~8-15, Full-failure ~12-15); a null there may be underpowering,
not absence of effect. Each panel title prints n_min/class.
'''
...
node_md={"cell_type":"markdown","metadata":{},"source":md}
node_code={"cell_type":"code","metadata":{},"execution_count":None,"outputs":[],"source":code}
if code_i is not None:
    nb["cells"][code_i]=node_code
    if md_i is not None: nb["cells"][md_i]=node_md
    print(f"Replaced TFCE-LDA cell at {code_i} (md at {md_i}).")
else:
    nb["cells"].append(node_md); nb["cells"].append(node_code); print("Appended new TFCE-LDA section.")
